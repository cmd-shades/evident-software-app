    ...
    */
    public function get_discipline_attendance_info($account_id = false, $discipline_id = false, $where = false)
    {
        ...
        if (!empty($account_id) && !empty($discipline_id)) {
            ...
            $this->db->select(
                'job_types.discipline_id, account_discipline.account_discipline_name `discipline_name`, SUM( CASE WHEN status_group IN ( "inprogress", "failed", "successful" ) THEN 1 ELSE 0 END) AS `completed_visits`,
				SUM(CASE WHEN job_id > 0 THEN 1 ELSE 0 END) AS `scheduled_visits`',
                false
            );
    ...
    public function get_discipline_outcomes_info($account_id = false, $discipline_id = false, $where = false)
    {
        ...
        if (!empty($account_id) && !empty($discipline_id)) {
            ...
            if (!empty($detailed_info)) {
                $this->db->select(
                    'audit.audit_id, audit.audit_status, audit.date_created, audit_result_statuses.result_status, audit_result_statuses.result_status_group `audit_outcome`, audit.job_id, audit_types.audit_type_id, audit_types.audit_type, audit_types.discipline_id, account_discipline.account_discipline_name `discipline_name`',
                    false
                )->group_by('audit.audit_id');
            } else {
                $this->db->select(
                    'audit_types.discipline_id, account_discipline.account_discipline_name `discipline_name`, 
					...
					SUM( CASE WHEN audit_id > 0 THEN 1 ELSE 0 END) AS `total_inspections`',
                    false
