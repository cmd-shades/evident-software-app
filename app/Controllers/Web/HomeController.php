...
use App\Adapter\FrontController;
...
class HomeController extends FrontController
{
    ...
    public function index()
    {
        ...
        $allowed_modules = $this->webapp_service->api_dispatcher($this->api_end_point.'access/check_module_access', [
        ...
        $data['asset_totals'] = $this->webapp_service->api_dispatcher($this->api_end_point.'statistics/asset_stats', [
			'account_id' => $this->account_id,
			'stat_type' => 'total_number'
		], $this->options, true);

        $data['building_compliance'] = $this->webapp_service->api_dispatcher($this->api_end_point.'audit/audit_stats', [
			'account_id'=>$this->account_id,
			'stat_type' => 'audit_results'
		], $this->options, true);

        $data['asset_eol'] = $this->webapp_service->api_dispatcher($this->api_end_point.'statistics/asset_stats', [
			'account_id'=>$this->account_id,
			'stat_type' => 'eol'
		], $this->options, true);

        $data['replacement_cost'] = $this->webapp_service->api_dispatcher($this->api_end_point.'statistics/asset_stats', [
			'account_id'=>$this->account_id,
			'stat_type' => 'replace_cost'
		], $this->options, true);
