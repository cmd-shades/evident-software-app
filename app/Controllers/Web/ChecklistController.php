...
class ChecklistController extends MX_Controller
{
	/**
    ...
    public function index()
    {
        ...
        if (!$this->user->is_admin && !$module_access) {
            ...
            redirect('webapp/checklist/overview', 'refresh');
        }
    }


    /** Get list of jobs **/
    public function checklist_jobs($job_id = false)
    {
        if ($job_id) {
        ...
        if (!$this->user->is_admin && !$module_access) {
            ...
            $data['selected_groups'] = !empty($this->input->get('group_status')) ? [$this->input->get('group_status')] : false;

            $data['group_statuses']	= [ 'completed'=>'Completed', 'in_progress'=>'In Progress', 'cancelled'=>'Cancelled', 'not_started'=>'Not Started',  ];
            $this->_render_webpage('checklists/index', $data);
        }
    }


    //View Job profile
    public function profile($job_id = false, $page = 'details')
    {
            ...
            if (!empty($data['job_details'])) {
                ...
                switch($page) {
                    case 'stock':
                        ...
                        $data['stock_items']	= (!empty($stock_items)) ? json_encode($stock_items) : false;
                    ...
                    case 'evidocs':
                        ...
                        $data['include_page'] = 'job_evidoc_details.php';
                        break;
    ...
    **/
    ...
    public function add_job_consumed_items()
    {
        ...
        if (!$this->user->is_admin && empty($item_access->can_add) && empty($item_access->is_admin)) {
            ...
            } elseif ($postdata['add_type'] == 'consumed_items') {
    ...
    **/
    ...
    //Manage Job types
    public function job_types($job_type_id = false, $page = 'details')
    {
        $toggled		= (!empty($this->input->get('toggled')) ? $this->input->get('toggled') : false);
    ...
    **/
    ...
    public function risks($risk_id = false, $page = 'details')
    {
        $toggled	= (!empty($this->input->get('toggled')) ? $this->input->get('toggled') : false);
        $section 	= (!empty($page)) ? $page : (!empty($this->input->get('page')) ? $this->input->get('page') : 'details');
    ...
    **/
    ...
    public function schedule_frequencies($frequency_id = false, $page = 'details')
    {
        $toggled		= (!empty($this->input->get('toggled')) ? $this->input->get('toggled') : false);
        $section 		= (!empty($page)) ? $page : (!empty($this->input->get('page')) ? $this->input->get('page') : 'details');
    ...
    */
    ...
    /** Manage Schedules **/
    public function new_schedule($contract_id = false, $site_id = false, $location_id = false, $asset_id = false)
    {
        ...
        $placeholders	= (!empty($this->input->get('placeholders')) ? $this->input->get('placeholders') : $placeholders);
        ...
        if (!empty($placeholders)) {
            ## Tessercact checklist Job Types
            ...
            $checklist_frequencies	 		= $this->webapp_service->api_dispatcher($this->api_end_point.'tesseract/checklist_frequencies', [ 'account_id'=>$this->user->account_id ], ['auth_token'=>$this->auth_token], true);
            $data['checklist_frequencies']	= (isset($checklist_frequencies->checklist_frequencies)) ? $checklist_frequencies->checklist_frequencies : null;
            ...
            $data['contract_buildings']	= (isset($contract_buildings->linked_sites_data)) ? $contract_buildings->linked_sites_data : null;
    ...
    public function schedule_profile($schedule_id = false, $page = 'schedules')
    {
        ...
        if (!$this->user->is_admin && empty($item_access->can_edit) && empty($item_access->is_admin)) {
            ...
            if (!empty($data['schedule_details'])) {
                ...
                $schedule_activities 		= $this->webapp_service->api_dispatcher($this->api_end_point.'job/schedule_activities', ['account_id'=>$this->user->account_id,'where'=>['schedule_id' => $schedule_id ] ], ['auth_token'=>$this->auth_token], true);
    ...
    */
    ...
    public function schedules($frequency_id = false, $page = 'details')
    {
        $data['rows'] = '';
    ...
    */
    private function load_schedules_view($schedules_data)
    {
        ...
        if (!empty($schedules_data)) {
            foreach ($schedules_data as $k => $schedule_details) {
                $return_data .= '<tr>';
                $return_data .= '<td>'. ucwords($schedule_details->schedule_name) .'</td>';
                ...
                $return_data .= '<td><a href="'.base_url('/webapp/job/schedule_profile/'.$schedule_details->schedule_id).'" ><i class="far fa-edit"></i> Open</a></td>';
                $return_data .= '</tr>';
            }
    ...
    public function stock($item_id = false, $page = 'details')
    {
        $toggled	= (!empty($this->input->get('toggled')) ? $this->input->get('toggled') : false);
        $section 	= (!empty($page)) ? $page : (!empty($this->input->get('page')) ? $this->input->get('page') : 'details');
    ...
    **/
    ...
    public function tracking_statuses($job_tracking_id = false, $page = 'details')
    {
        $toggled	= (!empty($this->input->get('toggled')) ? $this->input->get('toggled') : false);
        $section 	= (!empty($page)) ? $page : (!empty($this->input->get('page')) ? $this->input->get('page') : 'details');
    ...
    **/
    ...
    private function load_schedule_placeholders_view($schedule_data = false, $assets_data = false)
    {
        ...
        if (!empty($schedule_data) && !empty($assets_data)) {
            ...
            $dueDate			= date($schedule_data['due_date']);
            ...
            for ($i = 1; $i <= $activityCount; $i++) {
                ...
                foreach ($assets_data as $key => $assets_obj) {
                    ...
                    $return_data .= '<tr>';
                    $return_data .=  '<td colspan="4"  >&nbsp;&nbsp;&nbsp;<strong>'.strtoupper($assets_obj->asset_type->asset_type).' ('.$total_assets.')</strong><span class="pull-right" >Total Assets: '.$total_assets.' | To Be Checked: '.$total_assets_to_check.'</span></td>';
                    $return_data .= '</tr>';
                    $return_data .= '<tr>';
                    $return_data .=  '<td  colspan="4" >';
                    $return_data .= '<div class="col-md-12" >';
                    $looper = 0;
                    if ($total_assets > $max_count) {
                        $return_data .= '<div class="row" >';
                        ...
                        foreach ($assets_obj->assets as $k => $asset) {
                            $looper++;
                            $activity_name = ordinal($i).' Visit - ' .$activityName.' '.$looper;

                            $return_data .= '<input type="hidden" name="schedule_activities[multi]['.$i.']['.$asset->asset_id.'][asset_id]" value="'.$asset->asset_id.'" />';
                            $return_data .= '<input type="hidden" name="schedule_activities[multi]['.$i.']['.$asset->asset_id.'][activities_total]" value="'.$activityCount.'" />';
                            $return_data .= '<input type="hidden" name="schedule_activities[multi]['.$i.']['.$asset->asset_id.'][job_type_id]" value="'.$asset->job_type_id.'" />';
                            $return_data .= '<input type="hidden" name="schedule_activities[multi]['.$i.']['.$asset->asset_id.'][activity_name]" value="'.$activity_name.'" />';
                            $return_data .= '<input type="hidden" name="schedule_activities[multi]['.$i.']['.$asset->asset_id.'][proportion]" value="" />';
                            $return_data .= '<input type="hidden" name="schedule_activities[multi]['.$i.']['.$asset->asset_id.'][due_date]" value="'.$processedDueDate.'" />';
                            ...
                            if ($looper == $total_assets_to_check) {
                                break;
                            }
                        }
                    } else {
                ...
                $dueDate 		= date('d-m-Y', strtotime($dueDate.' '.(int)$activityInterval.' days'));
            }
            $return_data .= '</table>';
        }
        return $return_data;
    ...
    **/
    ...
    public function bom_categories($bom_category_id = false, $page = 'details')
    {
        $toggled		 = (!empty($this->input->get('toggled')) ? $this->input->get('toggled') : false);
        $section 		 = (!empty($page)) ? $page : (!empty($this->input->get('page')) ? $this->input->get('page') : 'details');
    ...
    public function fetch_assigned_jobs_data($job_date = false, $job_type_id = false, $region_id = false)
    {
        ...
        if (!empty($job_date)) {
            ...
            if (!empty($jobs_result)) {
                ...
                $engineer_resource	= !empty($engineers_result->{$job_date}) ? $engineers_result->{$job_date} : $result;
    ...
    private function load_assigned_jobs($assigned_jobs = false)
    {
        ...
        if (!empty($assigned_jobs)) {
            ...
            foreach ($assigned_jobs as $key => $engineer_details) {
                ...
                $return_str .= '<th width="20%" ><a class="pointer assigned-row text-normalweight" data-engineer_id="'.$engineer_details->engineer_id.'" >Total Jobs: '.(!empty($engineer_details->total_jobs) ? ($engineer_details->total_jobs) : 0).' </a></th>';
                $return_str .= '</tr>';
                ...
                if ($engineer_details->status_jobs) {
                    foreach ($engineer_details->status_jobs as $job_status => $status_jobs) {
                        foreach ($status_jobs as $status_key => $status_job) {
                            $return_str .= '<tr class="user-element no-border">';
                            $return_str .= '<td width="40%" class="no-border" style="padding-left:30px;">';
                            ...
                            $return_str .= '</label>';
                            $return_str .= '</td>';
                            $return_str .= '<td width="10%" class="no-border" >'.(date('d-m-Y', strtotime($status_job->job_date))).'</td>';
                            $return_str .= '<td width="40%" class="no-border" >'.$status_job->address_line_1.', '.$status_job->address_city.' '.$status_job->job_postcode.'</td>';
                            $return_str .= '<td width="10%" class="no-border" ><span class="pull-right" >'.$status_job->job_status.'</span></td>';
                            $return_str .= '</tr>';
                        }
                    }
                }
    ...
    */
    ...
    /*
    * Checklist Jobs Search
    */
    public function checklist_jobs_search($page = 'details')
    {
        $return_data = '';
        ...
        if (!$this->user->is_admin && !$module_access) {
            ...
            $search_result	= $this->webapp_service->api_dispatcher($this->api_end_point.'checklist/checklist_job_search', $postdata, ['auth_token'=>$this->auth_token], true);
            ...
            if (!empty($job)) {
                ...
                $return_data = $this->load_checklist_jobs_search_results($job);
                if (!empty($pagination)) {
    ...
    /*
    * Prepare Checklist jobs views
    */
    private function load_checklist_jobs_search_results($jobs_data = false)
    {
        $return_data = '';
        if (!empty($jobs_data)) {
            foreach ($jobs_data as $k => $job_details) {
                $return_data .= '<tr>';
                $return_data .= '<td><a href="'.base_url('/webapp/job/profile/'.$job_details->job_id).'/checklists" >'.$job_details->job_type.'</a> &nbsp;&nbsp;<small title="This is an uploaded Job" class="" style="font-size:80%; display:'.(($job_details->is_uploaded == 1) ? 'inline-block' : 'none').'"><i class="far fa-arrow-alt-circle-up"></i></small></td>';
                $return_data .= '<td><a href="'.base_url('/webapp/site/profile/'.$job_details->site_id).'/checklists" >'.$job_details->site_name.'</td>';
                $return_data .= '<td>'.$job_details->site_postcodes.'</td>';
                $return_data .= '<td>'.$job_details->site_reference.'</td>';
                $return_data .= '<td>'.((valid_date($job_details->due_date)) ? date('d-m-Y', strtotime($job_details->due_date)) : '').'</td>';
                $return_data .= '<td>'.(!empty($job_details->checklist_status) ? $job_details->checklist_status : '').'</td>';
                $return_data .= '<td>'.$job_details->job_status.'</td>';
                $return_data .= '<td>'.((valid_date($job_details->finish_time)) ? date('d-m-Y H:i:s', strtotime($job_details->finish_time)) : '').'</td>';
    ...
    private function load_search_results($search_results = false)
    {
        ...
        if (!empty($search_results)) {
            foreach ($search_results as $k => $job_details) {
                ...
                $return_data .= '<td>'.((!empty($job_details->postcode)) ? $job_details->postcode : "").'</td>';
                $return_data .= '<td>'.$job_details->assignee.'</td>';
                $return_data .= '<td>'.$job_details->job_status.'</td>';
        ...
        if (!empty($un_assigned_jobs)) {
            if (!empty($grouped)) {
                ...
                foreach ($un_assigned_jobs as $key => $job_type_details) {
                    ...
                    if ($job_type_details->jobs_list) {
                        foreach ($job_type_details->jobs_list as $job_key => $job_details) {
                            $return_str .= '<tr class="user-element no-border">';
                            $return_str .= '<td width="35%" class="no-border" style="padding-left:30px;">';
                            ...
                            $return_str .= '<td width="30%" class="no-border" >'.$job_details->address_line_1.', '.$job_details->address_city.' '.$job_details->job_postcode.'</td>';
                            $return_str .= '<td width="10%" class="no-border" >'.$job_details->job_status.'</td>';
                            $return_str .= '<td width="10%" class="no-border" ><span class="pull-right" >'.$job_details->job_duration.'</span></td>';
                            $return_str .= '</tr>';
                        }
                    }
                    $return_str .= '</table>';
                ...
                foreach ($job_type_details as $job_key => $job_details) {
                    $return_str .= '<tr class="user-element no-border">';
                    $return_str .= '<td width="35%" class="no-border">';
        ...
        if (!empty($schedule_data) && !empty($sites_data)) {
            ...
            for ($i = 1; $i <= $activityCount; $i++) {
                ...
                foreach ($sites_data as $key => $site_obj) {
                    ...
                    $return_data .= '<input type="hidden" name="schedule_activities[multi]['.$i.']['.$site_obj->site_id.'][due_date]" value="'.$processedDueDate.'" />';

                    $return_data .= '<div class="row" >';
                    $return_data .=  '<div class="col-md-3" >&nbsp;&nbsp;&nbsp;'.$site_obj->site_name.', '.strtoupper($site_obj->site_postcode).'</div>';
                    $return_data .=  '<div class="col-md-4" >'.urldecode($site_obj->evidoc_type).'</div>';
    ...
    private function load_checklist_search_results($checklists_data = false)
    {
        ...
        if (!empty($checklists_data)) {
            foreach ($checklists_data as $k => $checklist_details) {
                ...
                $return_data .= '<td>'.$checklist_details->site_name.' '.$checklist_details->site_postcodes.'</td>';
    ...
    **/
    ...
    public function download($job_id = false)
    {
        if ($job_id) {
            $postdata 			= ['account_id'=>$this->user->account_id, 'job_id'=>$job_id ];
            $checklist_record	= $this->webapp_service->api_dispatcher($this->api_end_point.'job/checklist_search', $postdata, ['auth_token'=>$this->auth_token], true);
            $checklist_data		= (isset($checklist_record->checklist)) ? $checklist_record->checklist : null;

            if (!empty($checklist_data)) {
                $checklist_uuid = $this->ssid_common->create_guid();
                $this->fetch_checklist_data($job_id, $checklist_uuid, $checklist_data);
            } else {
                redirect('webapp/audit/', 'refresh');
            }
        } else {
    ...
    /** fetch file **/
    public function fetch_checklist_data($job_id = false, $checklist_uuid = false, $checklist_data = false)
    {
        if (!empty($job_id) && !empty($checklist_uuid) && !empty($checklist_data)) {
            $contract_id 			 = (!empty($checklist_data->contract_id)) ? $checklist_data->contract_id : (!empty($checklist_data->linked_contract_id) ? $checklist_data->linked_contract_id : false);
            ...
            #Fetch any docuemnts
            $job_documents		 		= $this->webapp_service->api_dispatcher($this->api_end_point.'document_handler/document_list', ['account_id'=>$this->user->account_id, 'job_id'=>$job_id, 'document_group'=>'job' ], ['auth_token'=>$this->auth_token], true);
            $checklist_document			= (isset($job_documents->documents->{$this->user->account_id})) ? $job_documents->documents->{$this->user->account_id} : null;

            $checklist_data->checklist_documents = !empty($checklist_document) ? $checklist_document : false;
            ...
            $setup['document_setup'] = [
                ...
                'document_content'	=> $checklist_data,
                'generic_details'	=>[
                    'document_name'			=> (!empty($checklist_data->job_type)) ? $checklist_data->job_type : (!empty($audit_data->job_type) ? $audit_data->job_type : 'Checklist Record'),
                    'audit_frequency'		=> (!empty($checklist_data->frequency)) ? ' ('.$checklist_data->frequency.') ' : '',
                    'document_date'			=> date('l, jS F Y'),
                    'referrence_number'		=> '',
                    'custom_logo'			=> (!empty($custom_logo)) ? base_url($custom_logo) : false, //This link should be saved in the DB account configs!
                    'custom_log_dimensions'	=> 'width="50px"',
                    'custom_footer'			=> DOCUMENT_POWERED_BY,
                    'image_preview'			=> true,
                ],
            ];
            #debug( $setup);
            $this->ssid_common->create_pdf_from_template('evipdf/templates/checklist_pdf_template.php', $setup);
        } else {
            redirect('webapp/audit/', 'refresh');
        }
    }
