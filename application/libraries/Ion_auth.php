...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$this->load->model('serviceapp/ion_auth_model');
		...
		$this->_current_user = $this->ion_auth_model->_current_user();
	}
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		if( !empty($additional_data['user_type_id']) && ( in_array( $additional_data['user_type_id'], [1] )) ){
			$group_ids = [1,2];//Add to both admin and general user groups
		}
		...
		else
		{
			...
			## Remove this to force user to activate account via email
			$activate = $this->ion_auth_model->activate($id);
			
			$activation_code = ( $activate ) ? $this->ion_auth_model->activation_code : null;
			$identity        = $this->config->item('identity', 'ion_auth');
			...
			$data = array(
				...
				'first_name' => trim(ucwords(strtolower($user->first_name))),
				'last_name'  => trim(ucwords(strtolower($user->last_name))),
			...
			if( !$this->config->item('use_ci_email', 'ion_auth') ){
				$email_data = [
					'to'=>$email,
					'from'=>$email,
					'subject'=>$this->config->item('site_title', 'ion_auth') . ' - Account Registration',
					'message'=>$this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_activate', 'ion_auth'), $data, true)
				];
				
				$send = $this->mailer->send_mail( $email_data );
				
				$this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful', 'activation_email_successful'));
				$this->set_message('activation_email_successful');
				unset($data['activation'],$data['temp_password']);
				return $id;
			}else{
				...
				$this->email->subject($this->config->item('site_title', 'ion_auth') . ' - Account Registration');
				$this->email->message($message);
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		{
			$users_groups = $this->ion_auth_model->get_user_groups($id)->result();
			$groups_array = array();
	 ...
	 * @author Simply SID
	 **/
	public function logged_in( $user_id = false ){
		
		$this->ion_auth_model->trigger_events('logged_in');
                
		$recheck = $this->ion_auth_model->_check_loggin( $user_id );
	...
	public function is_account_holder( $id=false ){
		return $this->ion_auth_model->is_account_holder( $id );
	}
}
