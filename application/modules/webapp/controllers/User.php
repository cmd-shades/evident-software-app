...
class User extends MX_Controller {
					...
					if( in_array( $response->user->user_type_id, $this->ion_auth_model->no_web_access_allowed ) ){
						...
						if( $previous_url && ( preg_match( '(login|logout|change_password|forgot_password)', $previous_url ) != 1 ) ){
							...
							if( in_array( $response->user->user_type_id, EXTERNAL_USER_TYPES ) ){
								redirect( 'webapp/dashboard/index', 'refresh' );
							} else {
	...
	//View user profile
	function profile( $user_id = false, $page = 'details' ){
		...
		if( !$this->user->is_admin && empty( $item_access->can_view ) && empty( $item_access->is_admin ) ){
			...
			switch( $page ){
				...
				default:
					...
					$supervisors				= $this->webapp_service->api_dispatcher( $this->api_end_point.'user/users', ['account_id'=>$this->user->account_id, 'where'=> [ 'is_supervisor' => 1 ] ], ['auth_token'=>$this->auth_token] , true);
					$data['supervisors']		= ( isset( $supervisors->users ) ) ? $supervisors->users : null;
