...
class Diary extends MX_Controller {
	...
	public function progress(){
		...
		if( !$this->user->is_admin && !$module_access ){
            ...
            foreach( $all_user_jobs as $user_jobs ){
				...
				foreach( $user_jobs->assigned_jobs as $assigned_job ){
					foreach( $all_jobs as $scan_job ){
						if( $assigned_job->job_id == $scan_job->job_id ){
							...
							array_push( $schedule_context['totalJobs'],
								array(
				...
				array_push( $schedule_context['user_jobs_formatted'],
					array(
						'user_id' 			=> $user_id,
	...
	function manage_skills( $skill_id = false, $page = 'details' ){
	...
	*/
	...
	function manage_regions( $region_id = false, $page = 'details' ){
		...
		if( !$this->user->is_admin && empty( $item_access->can_add ) && empty( $item_access->is_admin ) ){
			...
			if( !empty( $job_id ) ){
				...
				if( !empty( $data['job_details'] ) ){
					...
					$params = array_merge( [ 'account_id'=>$this->user->account_id, 'where'=>[
							'date_from'			=> $date_from,
							'date_to'			=> $date_to,
