				...
				<div class="has-shadow x_panel">
						...
						<div class="col-md-4 col-sm-3 col-xs-12">
							...
							<div class="row">
								<div class="col-md-6 col-xs-12">
									<div class="checkboxs">
										<label class="pointer" for="check-book-jobs" ><input id="check-book-jobs" type="checkbox" /> <small>Book selected Jobs for this date</small></label>
									</div>
								</div>
					...
					<div class="row">
						<form id="un-assigned-jobs-list-form" >
							...
							<?php /* <input type="hidden" name="due_date" id="hidden_due_date" value="<?php echo date( 'd-m-Y', strtotime( $due_date ) ); ?>" /> */ ?>
							<input type="hidden" name="book_jobs" id="book_jobs" value="0" />
...
<script>
	$( document ).ready(function(){
		...
		load_jobs_data( contractID, jobDate, jobTypesArr, regionsArr, showBlankDates );
		
		function load_jobs_data( contractID, jobDate, jobTypesArr, regionsArr, showBlankDates ){
			...
			$.ajax({
				...
				method:"POST",
				data:{ contract_id:contractID, job_date:jobDate, job_type_id:jobTypesArr, region_id:regionsArr, include_blank_dates:showBlankDates },
				dataType: 'json',
		...
		$( function() {
			$( "#job_date" ).datepicker({
				...
				onSelect: function() {
					...
					load_jobs_data( contractID, pickedDate, jobTypesArr, regionsArr, showBlankDates );
				}
			});
		});
		...
		$( '#check-book-jobs' ).click( function(){
			if( $( this ).is(':checked') ){
				$( '#book_jobs' ).val( 1 );
				$( '[name="rebook_date"]' ).val( $( '[name="job_date"]' ).val() );
			} else {
				$( '#book_jobs' ).val( 0 );
				$( '[name="rebook_date"]' ).val( '' );
			}
		});
