			...
			<div class="x_content">
					...
					<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
						<div class="row">
							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
								<a href="<?php echo base_url('/webapp/job/new_category' ); ?>" class="btn btn-block btn-success success-shadow" title="Click to add a new BOM Category"><i class="fas fa-plus-circle" title="" style="font-size: 18px;"></i></a><br/>
							</div>
						</div>
					</div>
				...
				<div class="table-responsive alert alert-ssid alert-results" role="alert" style="overflow-y: hidden;" >
					<table id="datatable" class="table table-responsive" style="margin-bottom:0px;width:100%" >
						<thead>
							<tr>
								<th width="20%">BOM Category</th>
								<th width="25%">Category Description</th>
								<th width="45%">Category Group</th>
...
<script type="text/javascript">
	...
	$( document ).ready(function(){
		...
		var search_str   	= null;
		var category_groups_arr	= [];
		var start_index	 	= 0;
		var where 			= { 	
			'category_group':category_groups_arr
		};
		...
		function load_data( search_str, where, start_index ){
			$.ajax({
				url:"<?php echo base_url('webapp/job/bom_categories_list'); ?>",
				method:"POST",
		...
		function get_statuses( identifier ){
			...
			if( identifier == '.category-groups' ){
				...
				$( identifier ).each(function(){
					...
					if( $(this).is(':checked') ){
						totalChekd++;
						category_groups_arr.push( $(this).val() );
					}else{
						unChekd++;
					}
				});
