			...
			<div class="x_content">
					...
					<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
						<div class="row">
							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
								<a href="<?php echo base_url('/webapp/job/new_tracking_status' ); ?>" class="btn btn-block btn-success success-shadow" title="Click to add a new Tracking Status"><i class="fas fa-plus-circle" title="" style="font-size: 18px;"></i></a><br/>
							</div>
						</div>
					</div>
				...
				<div class="table-responsive alert alert-ssid alert-results" role="alert" style="overflow-y: hidden;" >
					<table id="datatable" class="table table-responsive" style="margin-bottom:0px;width:100%" >
						<thead>
							<tr>
								<th width="25%">Tracking Status</th>
								<th width="30%">Tracking Description</th>
								<th width="20%">Tracking Group</th>
...
<script type="text/javascript">
	...
	$( document ).ready(function(){
		...
		var search_str   			= null;
		var job_tracking_groups	= [];
		var start_index	 			= 0;
		var where 					= { 	
			'job_tracking_group':job_tracking_groups
		};
		...
		function load_data( search_str, where, start_index ){
			$.ajax({
				url:"<?php echo base_url('webapp/job/job_tracking_statuses'); ?>",
				method:"POST",
		...
		function get_statuses( identifier ){
			...
			if( identifier == '.region-groups' ){
				...
				$( identifier ).each(function(){
					...
					if( $(this).is(':checked') ){
						totalChekd++;
						job_tracking_groups.push( $(this).val() );
					}else{
						unChekd++;
					}
				});
