					...
					<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
						<div class="row">
							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
								<a href="<?php echo base_url('/webapp/premises/new_premises_attribute' ); ?>" class="btn btn-block btn-success success-shadow" title="Click to add a new Risk"><i class="fas fa-plus-circle" title="" style="font-size: 18px;"></i></a><br/>
							</div>
						</div>
					</div>
					...
					<table id="datatable" class="table table-responsive" style="margin-bottom:0px;width:100%" >
						<thead>
							<tr>
								...
								<th width="20%">Attribute Ref</th>
								<th width="20%">Response Type</th>
								<th width="20%">Response Options</th>
								<th width="20%"><span class="pull-right" >Status</span></th>
							</tr>
						</thead>
...
<script type="text/javascript">
	...
	$( document ).ready(function(){
		...
		var search_str   		= null;
		var premises_groups_arr	= [];
		var start_index	 		= 0;
		var where 				= { 	
			'premises_group':premises_groups_arr
		};
		...
		function load_data( search_str, where, start_index ){
			$.ajax({
				url:"<?php echo base_url( 'webapp/premises/attributes_lookup' ); ?>",
				method:"POST",
		...
		function get_statuses( identifier ){
			...
			if( identifier == '.premises-groups' ){
				...
				$( identifier ).each(function(){
					...
					if( $(this).is(':checked') ){
						totalChekd++;
						premises_groups_arr.push( $(this).val() );
					}else{
						unChekd++;
					}
				});
