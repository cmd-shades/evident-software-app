<div class="row">
	<?php if( $this->user->is_admin || !empty( $permissions->can_add ) || !empty( $permissions->is_admin ) ){ ?>
		<div class="col-md-6 col-sm-6 col-xs-12">
			<form id="docs-upload-form" action="<?php echo base_url("webapp/audit/upload_docs/".$audit_details->audit_id ) ?>" method="post" class="form-horizontal" enctype="multipart/form-data" >
				<input type="hidden" name="page" value="documents" />
				<input type="hidden" name="audit_id" value="<?php echo $audit_details->audit_id; ?>" />
				<input type="hidden" name="account_id" value="<?php echo $this->user->account_id; ?>" />
				<input type="hidden" name="document_group" value="<?php echo $document_group;?>" />
		...
		<div class="x_panel tile has-shadow">
			...
			<div class="row">
				<div class="col-md-12 table-responsive">								
					<?php if( !empty($audit_documents)){ foreach( $audit_documents as $file_group=>$files){ ?>
						...
						<?php foreach( $files as $k=>$file){ ?>
							<div class="row <?php echo str_replace( ' ', '', $file_group ); ?>" style="display:none;padding:5px 0">
								...
								<div class="col-md-2"><span class="pull-right"><a target="_blank" href="<?php echo $file->document_link; ?>"><i class="fas fa-download"></i></a> &nbsp;&nbsp;&nbsp;<span class="delete-file-btn pointer" data-document_id="<?php echo $file->document_id; ?>" ><i class="fas fa-trash-alt text-red"></i></span></span></div>
							</div>
						<?php }  ?>
					<?php } }  ?>
				</div>
	...
	$(document).ready(function(){
		...
		$( '.delete-file-btn' ).click( function( event ){
			...
			var documentId 		= $( this ).data( 'document_id' ),
				documentGroup 	= "<?php echo $document_group;?>";
			...
			swal({
				...
				if ( result.value ) {
					$.ajax({
						...
						method:"POST",
						data:{ page:'details', document_group:documentGroup, xsrf_token: xsrfToken, document_id:documentId },
						dataType: 'json',
