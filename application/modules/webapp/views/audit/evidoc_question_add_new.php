								...
								<?php if( !empty( $response_types ) ) { foreach( $response_types as $k => $resp_type ) { ?>
									...
									<?php } else if( ( $resp_type->response_type == 'radio' ) || ( $resp_type->response_type == 'checkbox' ) ){ ?>
										...
										<div class="resp_<?php echo $resp_type->response_type; ?> resp-type-options form-group" style="display:none">
											...
											<div class="form-group">
												<label>Which one of these options should trigger additional information?</label>
											...
											<div class="form-group">
												<label>Which one of these options is the Default Response?</label>
												<select id="default-response-<?php echo $resp_type->response_type; ?>" name="response_options[<?php echo $resp_type->response_type; ?>][default_response]" class="form-control default_response" >
													<option value="" >Select type</option>
													<?php if( !empty( $resp_type->response_type_options ) ) { foreach( $resp_type->response_type_options as $key => $select_resp_ops ) { ?>
														<option value="<?php echo $select_resp_ops->option_value; ?>" ><?php echo $select_resp_ops->option_value; ?></option>
													<?php } } ?>
											...
											<div class="form-group">
												<label>Is a Defects Response Required for this Question?</label>
												<div class="row">
													<div class="col-md-3 col-sm-6 col-xs-12">
														<div class="radio">
															<label><input type="radio" class="defects_response_required" name="defects_response_required" value="1" style="margin-top:6px;"> Yes</label>
														</div>
													</div>
													<div class="col-md-3 col-sm-6 col-xs-12">
														<div class="radio">
															<label><input type="radio" class="defects_response_required" name="defects_response_required" value="0" style="margin-top:6px;" checked > No</label>
														</div>
													</div>
												</div>
											</div>

											<div class="defects_response_required_container form-group" style="display:none">
												<div class="row">
													<div class="col-md-12 col-sm-12 col-xs-12">
														<label>Which one of these options should trigger a Defects Response?</label>
														<select id="defects-response-<?php echo $resp_type->response_type; ?>" name="response_options[<?php echo $resp_type->response_type; ?>][defects_response_trigger]" class="form-control defects_response_trigger" >
															<option value="" >Select type</option>
															<?php if( !empty( $resp_type->response_type_options ) ) { foreach( $resp_type->response_type_options as $key => $select_resp_ops ) { ?>
																<option value="<?php echo $select_resp_ops->option_value; ?>" ><?php echo $select_resp_ops->option_value; ?></option>
															<?php } } ?>
...
<script>
	...
	function addNewOption( optionType, optionValue ) {
		...
		if( optionType.length > 0 && optionValue.length > 0 ){
			...
			var defaultIdentifier = '#default-response-' + optionType;
			var defectsIdentifier = '#defects-response--' + optionType;
			
			var chkOptionExists  = ( $( selectIdentifier + ' option[value="' + optionValue + '"]').length > 0 );
			var deftOptionExists = ( $( defaultIdentifier + ' option[value="' + optionValue + '"]').length > 0 );
			var defectsOptExists = ( $( defectsIdentifier + ' option[value="' + optionValue + '"]').length > 0 );
			...
			if( !deftOptionExists ){
				$( defaultIdentifier ).append( $('<option >').val( optionValue ).text( ucwords( optionValue ) ) );
			}	
			
			if( !defectsOptExists ){
				$( defectsIdentifier ).append( $('<option >').val( optionValue ).text( ucwords( optionValue ) ) );
			}
	...
	$( document ).ready( function(){
			...
			$.ajax({
				...
				success:function(data){
					if( data.status == 1 ){
						...
						var sectionName = decodeURI(sectionName);
						
						$('#feedback_message').html( data.status_msg ).delay( 3000 ).fadeToggle( "slow" );
						
						var profileOptionExists 	= ( $('#section_id option[value="' + sectionName.replace( "&amp;", "&" ) + '"]').length > 0 );
						
						if( !profileOptionExists ){
							//Only add the new option if it doesn't already exist
							$('#section_id').append( $('<option >').val( sectionName.replace( "&amp;", "&" ) ).text( sectionName.replace( "&amp;", "&" ) ) );
						}
						
						//Set selected
						$('#section_id option[value="'+sectionName.replace( "&amp;", "&" )+'"]').prop( 'selected', true );
		...
		*/
		...
		$('.defects_response_required').on('change', function() {
			var checkListReq = $("input[name='defects_response_required']:checked").val();
			if( checkListReq == 1 ){
