		...
		<form id="evidoc-question-creation-form" >
			...
			<div class="evidoc_question_creation_panel2" style="display:none" >
						...
						<div class="resp-extra-options" style="display:none; margin:15px 0">
							...
							<div class="form-group resp-requirement" style="margin:15px 0">
								<?php if( !empty( $response_types ) ) { foreach( $response_types as $k => $resp_type ) { ?>
									...
									<?php } else if( $resp_type->response_type == 'input_integer' ){ ?>
										<div class="resp_<?php echo $resp_type->response_type; ?> resp-type-options" style="display:none; margin:15px 0">
											<div class="form-group">
									...
									<?php } else if( $resp_type->response_type == 'input_text' ){ ?>
										<div class="resp_<?php echo $resp_type->response_type; ?> resp-type-options" style="display:none; margin:15px 0">
											<div class="form-group">
									...
									<?php } else if( $resp_type->response_type == 'textarea' ){ ?>
									...
									<?php } else if( ( $resp_type->response_type == 'radio' ) || ( $resp_type->response_type == 'checkbox' ) ){ ?>
										...
										<div class="resp_<?php echo $resp_type->response_type; ?> resp-type-options form-group" style="display:none">
											...
											<div class="row checkbox-options opts-<?php echo $resp_type->response_type; ?>" data-checkbox_type="<?php echo $resp_type->response_type; ?>" >
												<?php if( !empty( $resp_type->response_type_options ) ) { foreach( $resp_type->response_type_options as $k => $resp_options ) { ?>
													<div class="col-md-4 col-sm-4 col-xs-12">
														<div class="radio">
															<label><input class="main-chk-options" type="checkbox" name="response_options[<?php echo $resp_type->response_type; ?>][options][]" value="<?php echo $resp_options->option_value; ?>" data-option_value="<?php echo $resp_options->option_value; ?>" style="margin-top:6px;" > <?php echo $resp_options->option_value; ?></label>
														</div>
													</div>
												<?php } } ?>
											</div>
											...
											<select id="extra-info-<?php echo $resp_type->response_type; ?>" name="response_options[<?php echo $resp_type->response_type; ?>][extra_info_trigger]" class="form-control extra_info_trigger" >
												...
												<?php if( !empty( $resp_type->response_type_options ) ) { foreach( $resp_type->response_type_options as $key => $select_resp_ops ) { ?>
													<option value="<?php echo $select_resp_ops->option_value; ?>" disabled ><?php echo $select_resp_ops->option_value; ?></option>
												<?php } } ?>
											</select>
				...
				<div class="row">
					...
					<div class="col-md-6 col-sm-6 col-xs-12">
						<button class="btn btn-block btn-flow btn-success btn-next evidoc-question-creation-steps validate-options" data-currentpanel="evidoc_question_creation_panel2" type="button" >Next</button>					
					</div>
				</div>
			</div>
						...
						<div class="col-md-11 col-sm-11 col-xs-11">
							<div class="form-group has-shadow" >
								<select id="section_id" name="section" class="form-control required" data-label_text="EviDocs Section" >
									...
									<?php if( !empty( $evidoc_type_sections ) ) { foreach( $evidoc_type_sections as $k => $section ) { ?>
			...
			<div class="evidoc_question_creation_panel6" style="display:none" >
				<div class="form-group">
				...
				<div class="form-group resp-requirement" style="margin:15px 0">
					<div class="row">
						<div class="col-md-6 col-sm-6 col-xs-12">
							<button class="btn btn-block btn-flow btn-back" data-currentpanel="evidoc_question_creation_panel6" type="button" >Back</button>					
						</div>
						<div class="col-md-6 col-sm-6 col-xs-12">
				...
				<div class="modal-content">
					...
					<div id="add-section-form-container" class="modal-body">
						<div class="row">
							...
							<div class="col-md-12 col-sm-12 col-xs-12">
								<button id="quick-add-section-btn" class="btn btn-block btn-success" type="button" >Add Section</button>					
							</div>
						</div>										
					</div>
				</div>
...
<script>
	...
	function addNewOption( optionType, optionValue ) {
		...
		if( optionType.length > 0 && optionValue.length > 0 ){
			...
			var elementCloner = '<div class="col-md-4 col-sm-4 col-xs-12"><div class="radio">';
				elementCloner += '<label>';
	...
	$( document ).ready( function(){
		...
		$( '.main-chk-options' ).each( function(){
			if ( !$( this ).is( ':checked' ) ){
				unChecked.push( $( this ).data( 'option_value' ) );
			}
		});
		
		$( '.main-chk-options' ).change( function(){
			var optValue = $( this ).data( 'option_value' );
			var respType = $( '#response_type_id option:selected' ).data( 'resp_type' );
			if ( $( this ).is( ':checked' ) ){
				
				unChecked.splice( $.inArray( optValue, unChecked ), 1 ); //Remove from unChecked array
				
				var variAble 	= $( this ).val().toString();
				respList.push( variAble );
				
				$( '#extra-info-'+respType+' option' ).each(function(){
					var chkVal = $( this ).val();
					if( $.inArray( chkVal, respList ) === -1 ){
						// respList.push( variAble );
						$( '#extra-info-'+respType ).not(':first').find('option[value!="'+chkVal+'"]').remove();
						console.log( 'Keep in list - '+ variAble );
					} else {
						console.log( 'Remove from list - '+ variAble );
						//$( '#extra-info-'+respType ).find('option[value!="'+variAble+'"]').remove();
						respList.splice( $.inArray( variAble, respList ), 1 );
				...
				$( '.main-chk-options' ).each( function(){
					if ( !$( this ).is( ':checked' ) ){
						unChecked.push( optValue );
					}
				});
				
				respList.splice( $.inArray( optValue, respList ), 1 );
		...
		// Section Quick add
		$( '#quick-add-section-btn' ).click(function(){
			...
			$.ajax({
				...
				success:function(data){
					if( data.status == 1 ){
						...
						var optionExists = ( $('#section_id option[value="' + sectionName + '"]').length > 0 );
		...
		$(".evidoc-question-creation-steps").click(function(){
			...
			if( $( this ).hasClass( 'validate-options' ) ){
				var isValid = validate_options();
				if( !isValid ){
					return false;
				}
			}
		...
		*/
		...
		function validate_options(){
			var respType 	= $( '#response_type_id option:selected' ).data( 'resp_type' );
			if( $.inArray( respType.toLowerCase().toString(), ['radio','checkbox', 'file', 'sinature'] ) !== -1 ){
				var checKed = $( ".resp_"+respType+" input:checked" ).length;
				if ( checKed < 1 ){
					swal({
						type: 'error',
						text: 'Please tick at-least 1 option for your users to choose from!'
					})
					return false;
				}

				var extraInf = $( '#extra-info-'+respType+' option:selected' ).val();
				if( extraInf.length == 0 ){
					swal({
						type: 'error',
						text: 'Please set the extra-information trigger!'
					})
					return false;
				}
			}
