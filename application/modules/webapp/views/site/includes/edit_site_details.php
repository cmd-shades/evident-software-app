					...
					if( !empty( $distribution_groups ) ){ ?>
						<select name="site_details[distribution_group_id]" class="input-field-60">
							...
							<?php foreach( $distribution_groups as $row ){?>
								<option value="<?php echo ( !empty( $row->setting_id ) ) ? $row->setting_id : ''; ?>" title="<?php echo ( !empty( $row->value_desc ) ) ? $row->value_desc : '' ?>" <?php echo ( ( !empty( $site_details->distribution_group_id ) && ( $site_details->distribution_group_id == $row->setting_id ) ) ? 'selected="selected"' : '' ); ?>><?php echo ( !empty( $row->setting_value ) ) ? $row->setting_value : '' ; echo ( !empty( $row->country ) ) ? '('.ucfirst( $row->country ).')' : '' ?> </option>
							<?php } ?>
						</select>
					<?php
					} else { ?>
