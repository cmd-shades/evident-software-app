<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel tile has-shadow">
			<legend>Asset Audits</legend>
			<?php if( !empty( $asset_audits ) ){ ?>
				<table style="width:100%">
					...
					<?php if( !empty( $asset_audits ) ){ foreach( $asset_audits as $audit_record ) { ?>
						<tr>
							<td><a href="<?php echo base_url( 'webapp/audit/profile/'.$audit_record->audit_id ); ?>"><?php echo $audit_record->audit_type; ?></a></td>
							<td><?php echo valid_date( $audit_record->evidoc_completion_date ) ? date( 'd-m-Y H:i:s', strtotime( $audit_record->evidoc_completion_date ) ) : ''; ?></td>
							...
							<td class="question-toggler pointer" data-audit_id="<?php echo $audit_record->audit_id; ?>" data-toggle="modal" data-target="#questions-modal-md" ><span class="pull-right text-blue text-bold" >View responses</span></td>
					...
					<div class="modal-dialog modal-lg">
						<div class="modal-content">
							<div class="modal-header"><button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span></button>
								<h4 class="modal-title" id="myModalLabel">Asset Audit Log</h4>
							</div>
