				...
				<div class="row" >
					<div class="col-md-6 col-sm-6 col-xs-12">
						...
						<?php if( $this->user->is_admin || !empty( $permissions->is_admin ) || !empty( $tab_permissions->can_edit ) ){ ?>
							<div class="input-group form-group">
								<label class="input-group-addon">Tracking Status (TSG)</label>
								<select id="job_tracking_id" name="job_tracking_id" class="form-control" >
									<option value="">Please select</option>
									<?php if( !empty( $tracking_statuses ) ) { foreach( $tracking_statuses as $k => $tracking_status ) { ?>
						...
						<div class="input-group form-group">
							...
							<select class="form-control" <?php echo ( $this->user->is_admin || !empty( $permissions->is_admin ) || !empty( $tab_permissions->is_admin ) ) ? 'name="eta_status"' : 'readonly'; ?> >
								<option value="" >Please select</option>
								<?php if( !empty( $eta_statuses ) ){ foreach( $eta_statuses as $k => $eta_status ){ ?>
							...
							<div class="failure-details" style="display:none" >
								<div class="input-group form-group">
									...
									<select id="fail_code_id" name="fail_code_id" class="form-control">
										<option value="" >Please select</option>
										<?php if( !empty( $fail_codes ) ) { foreach( $fail_codes as $k => $fail_code ) { ?>
						...
						<div id="job_notes" style="display:none" class="form-group" >
							<div class="input-group form-group">
								<label class="input-group-addon">Status Change Notes</label>
								<textarea name="job_notes" type="text" class="form-control has-error" rows="4"></textarea>
							</div>
