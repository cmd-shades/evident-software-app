		...
		<div class="x_panel tile shadow">
			<div class="rows">
				...
				<div class="control-group form-group">					
					<table>
						<tr>
							<th class="width_60">Building ID</th>
							<th class="width_160">Building Name</th>
							<th class="width_240">Building Address</th>
							<th class="width_80">Building Reference</th>
							<th class="width_80">Building Postcodes</th>
							<th class="width_120">Building Created</th>
							<th class="width_120">Building Linked By</th>
								...
								<tr>
									...
									<td data-label="Unlink Site" class="width_80">
										<?php if( $this->user->is_admin || !empty( $permissions->can_delete ) || !empty( $permissions->is_admin ) ){ ?>
											<a class="unlink_site_btn" onclick="return confirm( 'Are you sure you want to Unlink the site from the Contract?' );" href="<?php echo base_url( "webapp/contract/unlink_site/".$row->site_id."/".$profile_data[0]->contract_id."/linked_sites" ); ?>">Unlink Building</a>
										<?php } ?>
									</td>
								</tr>
							<?php
			...
			<div class="rows" style="margin-top: 10px;">
				<a class="btn btn-sm btn-flow btn-success btn-next pull-right no_right_margin" id="linkSitesWFButton">Link Building to this Contract &nbsp;<i class="fas fa-chevron-down"></i></a>
			</div>
		</div>
		<?php } else { ?>
			<div class="x_panel tile shadow">
				<div class="row">
					<div class="col-md-6 col-sm-6 col-xs-12 pull-left">
						<div class="row">
							<div class="col-md-6 col-sm-6 col-xs-12 pull-left">
								<legend>No Buildings linked to this Contract</legend>
							</div>
						</div>
						<div class="row">
							<div class="col-md-6 col-sm-6 col-xs-12 pull-left">
						
								<a class="btn btn-sm btn-flow btn-success btn-next pull-left" id="linkSitesWFButton">Link Building to this Contract &nbsp;<i class="fas fa-chevron-down"></i></a>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="x_panel" style="border-bottom: none;">
				<div class="rows">
					<form action="<?php echo base_url( "webapp/contract/link_sites" ) ?>" method="post" name="linkSitesWFForm" id="linkSitesWFForm">
						...
						<div class="row">
							<div class="col-md-12 col-sm-12 col-xs-12">
								<legend>Link Buildings to this Contract</legend>
							</div>
						</div>
						<div class="row">
							<div class="col-md-6 col-sm-6 col-xs-12">
								<div class="control-group form-group">
									<div class="controls">
										<label>Find the Building by typing the Building Name:</label>
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<div class="control-group form-group">
									<div class="controls">
										<label>Provide the Building ID. Multiple ID's must be comma separated:</label>
						...
						<?php if( $this->user->is_admin || !empty( $permissions->can_add ) || !empty( $permissions->can_edit ) || !empty( $permissions->is_admin ) ){ ?>
							<div class="row" style="margin-bottom: 6px;">
								<div class="col-md-6 col-sm-12 col-xs-12 pull-right">
									<button type="submit" class="btn btn-sm btn-flow btn-primary btn-success pull-right no_right_margin" id="linkSites">Link Buildings</button>
								</div>
							</div>
						<?php } else { ?>
							<div class="row" style="margin-bottom: 6px;">
...
<script>
	$( document ).ready( function(){
		...
		function load_data( search_str, start_index ){
			$.ajax({
				url:"<?php echo base_url( 'webapp/contract/sites_lookup' ); ?>",
				method:"POST",
				dataType: 'json',
