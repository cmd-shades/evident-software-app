...
<div class="row" style="">
	<div class="x_content">
		<div class="col-md-12 col-sm-12 col-xs-12 module-header">
			<h3><strong>Contract Home</strong></h3>
		</div>

		<!-- BUILDING COMPLIANCE -->
		<div class="col-md-3 col-sm-6 col-xs-12">
			<div class="x_title no-border text-center" >
				<h2 id="siteComplianceLabel"><small>Building Compliance</small></h2>
				<div class="clearfix"></div>
			</div>
			<div class="x_panel border-radius-10 has-shadow stats-container">
				<div class="x_content">
					<canvas id="siteCompliance"></canvas>
                    <div class="stat-error"></div>
					<br/>
					<div id="siteComplianceBars" class="data-bars"></div>
				</div>
			</div>
        </div>
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
			<div class="x_title no-border text-center">
				<h2 id="exceptionStatusesLabel"><small>Assets by Group</small></h2>
				<div class="clearfix"></div>
			</div>
			<div class="x_panel border-radius-10 has-shadow stats-container">
				<div class="x_content">
					<canvas id="barchart-8"></canvas>
                    <br/>
                    <div class="stat-error"></div>
				</div>
			</div>
		...
		<div class="col-md-3 col-sm-6 col-xs-12">
			<div class="x_title no-border text-center" >
				<h2 id="assetEOLstatsLabel"><small>END OF LIFE</small></h2>
				<div class="clearfix"></div>
			</div>
			<div class="x_panel border-radius-10 has-shadow stats-container">
				<div class="x_content">
					<div id="assetEOLstatsHeader" class="text-center" style="font-size:18px;"></div>
					<div id="assetEOLstats" class="text-center" style="font-size:18px; margin-bottom:30px"></div>
                    <div id="AssetEOLBars" class="data-bars"></div>
                    <div class="stat-error"></div>
                </div>
			</div>
        </div>

		<div class="row clear-both">&nbsp;</div>

		<div class="col-md-6 col-sm-12 col-xs-12">
			<div class="x_title no-border text-center">
				<h2 id="atitle"><small>Job Status</small></h2>
				<div class="clearfix"></div>
			</div>
			<div class="x_panel border-radius-10 has-shadow stats-container">
				<div class="x_content">
                    <canvas id="barchart-2" style="margin-top: 0;"></canvas>
                    <div class="stat-error"></div>
				</div>
			</div>
		</div>

		<!-- JOB COMPLETITION RATES -->
		<div class="col-md-6 col-sm-12 col-xs-12">
			<div class="x_title no-border text-center">
				<h2 id="atitle"><small>Job Completion Rates</small></h2>
				<div class="clearfix"></div>
			</div>
			<div class="x_panel border-radius-10 has-shadow stats-container">
				<div class="x_content">
					<canvas id="barchart-3" style="margin-top: 0;"></canvas>
                    <div class="stat-error"></div>
				</div>
			</div>
        </div>
        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
			<div class="x_title no-border text-center">
				<h2 id="atitle"><small>Schedules</small></h2>
				<div class="clearfix"></div>
			</div>
			<div class="x_panel border-radius-10 has-shadow stats-container">
	...
	$( document ).ready(function () {
		...
		function loadStatusChart(json_data, canvas_id) {
            ...
            if(dataAvaliable){
                ...
                $( "#" + canvas_id ).parent().find( ".stat-error" ).html( "<i class='fas fa-exclamation-circle fa-6x'></i><br><br><h4>No data is avaliable!</h4>" )
            }
		}
		...
		var contractID = <?php echo !empty($contract_id) ? $contract_id : -1; ?>
		...
		$.ajax({
			...
			method:"POST",
			data:{ account_id:accountID, stat_type:"audit_results", where : {'contract_id' : contractID} },
			dataType: 'json',
        ...
        $.ajax({
			...
			method:"POST",
			data:{account_id:accountID,stat_type:"completion", where : {'contract_id' : contractID}},
			dataType: 'json',
			success:function(result){
				if( result.status == 1 ){
        ...
        $.ajax({
			...
			method:"POST",
			data:{account_id:accountID,stat_type:"action_status", where : {'contract_id' : contractID}},
			dataType: 'json',
			success:function( result ){
				if( result.status == 1 ){
		...
		$.ajax({
			...
			method: "GET",
			data:{account_id:accountID, where : {'contract_id' : contractID}},
			dataType: 'json',
			success:function( result ){
				if( result.status == 1 ){
		...
		$.ajax({
			...
			method: "GET",
			data:{ account_id:accountID,where:{ data_target:'bar', 'contract_id' : contractID} },
			dataType: 'json',
			success:function(result){
				if( result.status == 1 ){
		...
		$.ajax({
			...
			success:function(result){
				if( result.status == 1 ){
					loadScheduleChart2( result, "barchart-12");
				}else{
					console.log( result.status_msg );
				}
			}
		...
		$.ajax({
			...
			method:"POST",
			data:{ account_id:accountID,stat_type:"audit_result_status", where : {'contract_id' : contractID} },
			dataType: 'json',
			success:function( result ){
				if( result.status == 1 ){
		...
		$.ajax({
			...
			method:"POST",
			data:{ account_id:accountID,stat_type:"outcome_result", where : {'contract_id' : contractID} },
			dataType: 'json',
			success:function( result ){
				if( result.status == 1 ){
		...
		$.ajax({
			...
			method:"POST",
			data:{ account_id:accountID, stat_type:"eol", where : {'contract_id' : contractID} },
			dataType: 'json',
			success:function( result ){
				if( result.status == 1 ){
					setChart( result, 'EolStats' );
					//loadExpiredChart( result, "barchart-1");
				}else{
					console.log( result.status_msg );
				}
			}
		...
		$.ajax({
			url:"<?php echo base_url( 'webapp/home/asset_stats' ); ?>",
			method:"POST",
			data:{ account_id:accountID, stat_type:"replace_cost", where : {'contract_id' : contractID} },
			dataType: 'json',
			success:function( result ){
				if( result.status == 1 ){
					var repairCost = '0.00';
					...
					$( '#assetEOLstats' ).html( '<small><i>Data not available</i></small>' );
					console.log( result.status_msg );
				}
			}
		});
		...
		$.ajax({
			url:"<?php echo base_url( 'webapp/home/audit_stats' ); ?>",
			method:"POST",
			data:{ account_id:accountID, stat_type:"periodic_audits", where : {'contract_id' : contractID} },
			dataType: 'json',
			success:function( result ){
				if( result.status == 1 ){
					setChart( result, 'periodicAudits' );
				}else{
					console.log( result.status_msg );
				}
			}
		...
		/*--- GET ASSET STATUS STATS --*/
		// $.ajax({
			// url:"<?php echo base_url('webapp/home/asset_stats'); ?>",
			// method:"POST",
			// data:{account_id:1,stat_type:"asset_status"},
			// dataType: 'json',
			// success:function(result){
				// if( result.status == 1 ){
					// setChart( result, 'AssetStatus' );
				// }else{
					// console.log( result.status_msg );
				// }
			// }
		...
		if( chartType.length > 0 ){
			...
			switch( chartType.toLowerCase() ){
                case 'eviexceptionstatus':
                    ...
                    if(data.stats){
                        $.each( data.stats, function( index, statusItem ) {
							...
							var clickFilterUrl	= "<?php echo base_url( '/webapp/audit/action_status' ); ?>";
                    ...
                    if( dataAvailable ){
                        ...
                        $( '#eviExceptionStatus' ).parent().find( ".stat-error" ).html( "<i class='fas fa-exclamation-circle fa-6x'></i><br><br><h4>No data is avaliable!</h4>" )
                    }
                ...
                case 'assetcompliance':
                    ...
                    var dataAvailable = false;
                        ...
                        if( dataAvailable ){
                            ...
                            $( '#assetCompliance' ).parent().find( ".stat-error" ).html( "<i class='fas fa-exclamation-circle fa-6x'></i><br><br><h4>No data is avaliable!</h4>" )
                        }
                ...
                case 'evidocresults':
                    ...
                    if(data.stats){
                        $.each( data.stats, function( index, statusItem ) {
                            if( (statusItem.status_total > 0 && statusItem.status_name != "total") || SHOW_EMPTY_FIELDS){
                                var statusBarDiv = '<a href="'+clickFilterUrl+'?group='+statusItem.result_status_group+'"><div class="box-header status-bar shadow-red" style="background-color:' + statusItem.hex_colour + '; opacity:0.9;color:#fff"><h3 class="status-bar-header">';
                    ...
                    if( total !== -1 && completed !== -1 && completed !== 0 ){
					...
					if( dataAvailable ){
                        ...
                        $( '#evidocResults' ).parent().find( ".stat-error" ).html( "<i class='fas fa-exclamation-circle fa-6x'></i><br><br><h4>No data is avaliable!</h4>" )
					}
				...
				case 'sitecompliance':
					...
					if( dataAvailable ){
                        ...
                        $( '#siteCompliance' ).parent().find( ".stat-error" ).html( "<i class='fas fa-exclamation-circle fa-6x'></i><br><br><h4>No data is avaliable!</h4>" )
				...
				case 'periodicauditss':
					...
					var dataStats		= data.stats,//Data totals
						...
						if( dataStatsTotals ){
                            ...
                            $( '#periodicAudits' ).parent().find( ".stat-error" ).html( "<i class='fas fa-exclamation-circle fa-6x'></i><br><br><h4>No data is avaliable!</h4>" )
						}
				...
				case 'auditresults':
					...
					if( dataAvailable ){
                        ...
                        $( '#auditResults' ).parent().find(".stat-error").html("<i class='fas fa-exclamation-circle fa-6x'></i><br><br><h4>No data is avaliable!</h4>")
					}
				...
				case 'eolstats':
                        ...
                        if(!dataAvaliable){
                            ...
                            $( '#assetEOLstats' ).hide();
                            $( '#AssetEOLBars' ).parent().find( ".data-bars" ).hide();
                            $( '#AssetEOLBars' ).parent().find( ".stat-error" ).html( "<i class='fas fa-exclamation-circle fa-6x'></i><br><br><h4>No data is avaliable!</h4>" )
                        }
