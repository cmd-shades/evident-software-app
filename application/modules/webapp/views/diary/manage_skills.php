					...
					<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
						<div class="row">
							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
								<a href="<?php echo base_url('/webapp/diary/new_skill' ); ?>" class="btn btn-block btn-success success-shadow" title="Click to add a new Skill"><i class="fas fa-plus-circle" title="" style="font-size: 18px;"></i></a><br/>
							</div>
						</div>
					</div>
					...
					<table id="datatable" class="table table-responsive" style="margin-bottom:0px;width:100%" >
						<thead>
							<tr>
								...
								<th width="50%">Skill Description</th>
...
<script type="text/javascript">
	...
	$( document ).ready(function(){
		...
		var search_str   	= null;
		var skill_groups_arr	= [];
		var start_index	 	= 0;
		var where 			= { 	
			'skill_group':skill_groups_arr
		};
		...
		function load_data( search_str, where, start_index ){
			$.ajax({
				url:"<?php echo base_url('webapp/diary/skills_bank'); ?>",
				method:"POST",
		...
		function get_statuses( identifier ){
			...
			if( identifier == '.skill-groups' ){
				...
				$( identifier ).each(function(){
					...
					if( $(this).is(':checked') ){
						totalChekd++;
						skill_groups_arr.push( $(this).val() );
					}else{
						unChekd++;
					}
				});
