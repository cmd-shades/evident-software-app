...
class Account_model extends CI_Model {
			...
			if( !empty( $account ) ){
				if( ( ( !empty($this->ion_auth->_current_user()) && ( (int)$account->account_id == (int)$this->ion_auth->_current_user()->account_id ) ) ) || $ignore_token ){
					...
					if( $account->is_active != 1 ){
						...
						$result = false;
					}else if( ( $status == 'Active' ) || ( $status == 'Trial' && ( $trial_period > $date_time_now ) ) ){
						$result = true;
						$this->session->set_flashdata('message','Account verified successfully.');
					}else if( $status == 'Trial' && ( $trial_period < $date_time_now ) ){
	...
	*/
	...
	function check_unique_accoount_email( $account_id = false, $account_email = false ) {
	...
	function get_system_modules( $user_id = false, $account_id = false, $admin_account_id = false, $module_id = false){
	...
	function update_base_module( $user_id, $account_id, $admin_account_id, $module_id = false, $module_data = false ){
		$result = false;
	...
	function fetch_all_tables( $user_id, $account_id, $admin_account_id ){
		$result = false;
	...
	function get_table_columns( $user_id, $account_id, $admin_account_id, $table_name = false ){
		$result = false;
	...
	function update_config_table( $user_id, $account_id, $admin_account_id, $config_data ){
	...
	function delete_config_entry( $user_id, $account_id, $admin_account_id, $config_entry_id ){
		$result = false;
