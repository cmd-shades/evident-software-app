...
class Modules_model extends CI_Model {
	...
	public function create_user_module_access( $user = false, $modules = array() ){
		...
		if( $user && $modules ){
			...
			if( !empty( $current_module_access ) ){
				...
				if( ( !empty( $this->ion_auth->_current_user()->id ) && ( $user->id == $this->ion_auth->_current_user()->id ) ) || ( $user->is_account_holder != 1 ) ){
			...
			foreach( $modules as $k=>$module ){
				...
				## Set tab permissions
				$mod_items = $this->db->select( 'module_item_id' )->get_where( 'user_module_items', [ 'module_id'=>$module ] );
				if( $mod_items->num_rows() > 0 ){
					
					$item_perm = [
						'user_id'			=> $user->id,
						'module_id'			=> $module,
						'account_id'		=> $user->account_id,
						'can_view'			=>	1,	# 1 - original value
						'can_add'			=> !empty( $is_admin ) ? 1 : 0,	# 1 - original value
						'can_edit'			=> !empty( $is_admin ) ? 1 : 0,	# 1 - original value
						'can_delete'		=> !empty( $is_admin ) ? 1 : 0,	# 0
						'is_admin'			=> !empty( $is_admin ) ? 1 : 0,		# 0
						'last_modified_by'	=> $this->ion_auth->_current_user->id
					];
					
					foreach( $mod_items->result() as $perm ){
						$item_perm['module_item_id'] = $perm->module_item_id;
						if( !empty( $is_admin ) ){
							$item_perm['is_admin'] 	 = 1;
							$item_perm['can_delete'] = 1;
						}
						$this->db->insert( 'user_module_item_permissions', $item_perm );
					}
				}
			}
	...
	**/
	public function get_module_access( $account_id=false, $user_id=false , $module_id = false, $app_uuid = false ){
		...
		if( $account_id && $user_id ){
			...
			$this->db->select('uma.user_id `user_id`, uma.module_id, uma.has_access, uma.is_module_admin, uma.has_mobile_access, um.module_name, um.module_ranking, mc.category_id, mc.category_name')
	...
	**/
	public function set_module_item_permissions( $user = false, $module_id = false, $item_permissions = array() ){
		...
		if( !empty( $user['id'] ) && !empty( $module_id ) ){
			...
			$insert_data = array_merge( ['user_id'=>$user['id'],'account_id'=>$user['account_id'],'module_id'=>$module_id,'last_modified_by'=>$this->ion_auth->_current_user->id], $item_permissions );
	...
	**/
	public function get_module_item_permissions( $account_id = false, $user_id = false, $module_id = false, $module_item_id = false, $module_item = false, $as_list = false ){
		...
		if( $account_id && $user_id ){
			...
			if( $module_item ){
				$module_item_record = $this->get_module_items( $account_id, false, $module_id, $module_item );
			...
			$this->db->select('mp.*,mi.module_item_tab,mi.module_item_name,m.module_name')
	...
	* 	Get module items
	**/
	public function get_module_items( $account_id = false, $item_id=false, $module_id = false, $module_item_name = false ){
		...
		if( !empty( $account_id ) ){
			...
			if( $query->num_rows() > 0 ){
				$this->session->set_flashdata( 'message', 'Module(s) items found.' );
				$result = $query->result();
			} else {
				$this->session->set_flashdata( 'message', 'No module items found.' );
			}
		} else {
			$this->session->set_flashdata( 'message', 'Account ID is missing.' );
		}
	...
	*/
	public function get_allowed_modules( $account_id=false, $user_id=false , $module_id=false, $app_uuid = false, $as_list = false ){
		...
		if( $account_id && $user_id ){
			...
			$this->db->select('uma.user_id `user_id`, uma.module_id, uma.has_access, uma.is_module_admin, mc.category_name, um.*')
			->join('user_module_categories mc','mc.category_id = um.category_id','left')
			->join('user_module_access uma','uma.module_id = um.module_id')
			->where("uma.user_id",$user_id)
			->where("uma.account_id",$account_id)
	...
	*/
	public function get_modules( $module_id = false, $active_only = true, $basic = true ){	
		$result = [];
		
		if( $module_id ){
			$this->db->where( 'um.module_id', $module_id );
		}
		...
		if( !empty( $basic ) ){
			$this->db->select('um.module_id, um.module_controller, um.module_name, um.description, um.module_price, um.module_price_management, um.module_price_intelligence,   mc.category_name, um.is_active');
		}else{
			$this->db->select('um.*, mc.category_id, mc.category_name');
		}

		$query = $this->db->join('user_module_categories mc','mc.category_id = um.category_id')
		...
		if( !empty( $account_id ) && !empty( $user_id ) ){
			if( !empty(  $data['permissions'] ) ){
				...
				foreach( $data['permissions'] as $module_id => $mod_permissions ){
					...
					}else if( $mod_permissions['full_access'] == 'no' || $mod_permissions['full_access'] == 0 || empty( $mod_permissions['module_items'] ) ){
					...
					}else if( $mod_permissions['full_access'] == 'yes' || $mod_permissions['full_access'] == 1 ) {
						$grant_access = $this->update_module_access( $account_id, $user_id, ['module_id'=>$module_id, 'has_access'=>1 ]);
						if( $grant_access ){
							$message= 'Module access updated successfully';
							$result = true;
