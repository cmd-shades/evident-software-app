								...
								if( !empty( $site ) ){
									...
									$viewings_per_hotel = count(
										array_filter( $csvAsArray, function( $x ){
											$array_counter = count( array_filter( $x, function( $x ) { return !empty( $x ); } ) );
									...
									if( !empty( $daily_rate_settings_exists ) ){
										foreach( $daily_rate_settings_exists as $set ){
											...
											} else if( !empty( $set->type_group ) && ( strtolower( $set->type_group ) == "revenue_share" ) ){
									...
									foreach( $csvAsArray as $movie_row ){
										...
										## to skip empty rows
										$arr_values = count( array_filter( $movie_row, function( $x ) { return !empty( $x ); } ) );
										if( $arr_values == 0 ) continue;
					...
					if( $type_id == 11 ){
						...
						foreach( $result as $key => $row ){
							...
							if( $query->num_rows() > 0 ){
								...
								$ad 							= array_map( function( $item ){ return str_pad( $item, 2, '0', STR_PAD_LEFT ); }, $ac );
					...
					if( $type_id == 9 ){
						foreach( $result as $key => $row ){
							...
							if( !empty( $row['product_type'] ) ){
								...
								} else if( in_array( strtolower( $row['product_type'] ), ['vod','linear']  ) ){
									$result[$key]['product_currency'] = $result[$key]['site_currency'];
								} else if( in_array( strtolower( $row['product_type'] ), ['ad hoc service']  ) ){
									$result[$key]['product_currency'] = $result[$key]['site_currency'];
								} else {
