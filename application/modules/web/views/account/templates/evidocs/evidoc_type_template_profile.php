				...
				<div class="row alert alert-ssid bg-blue" role="alert">
					<div class="profile-overview">
						<div class="col-md-12 col-sm-12 col-xs-12">
							<legend>EviDoc Type ID (<?php echo $evidoc_type_details->audit_type_id; ?>) <span class="hide pull-right"><span class="edit-evidoc-name pointer" title="Click to edit thie Evidoc Name profile"><i class="fas fa-pencil-alt"></i></span> &nbsp; <span class="delete-evidoc-type pointer" title="Click to delete this Evidoc Name profile" ><i class="far fa-trash-alt"></i></span></span></legend>
						</div>
						...
						<div class="col-md-5 col-sm-5 col-xs-12">
							<table style="width:100%;">
								...
								<tr>
									<td width="25%"><label>Template Owned By</label></td>
									<td width="75%"><?php echo (!empty($evidoc_type_details->account_name)) ? ucwords($evidoc_type_details->account_name) : ''; ?></td>
								</tr>
							</table>							
						</div>
				...
				<div class="row">
					<div class="col-md-12 col-sm-12 col-xs-12">
						...
						<div class="x_panel tile has-shadow">
							<legend>Question Bank <span class="hide pull-right pointer add-new-question"><i class="fas fa-plus" title="Add new Question to this EviDoc type" ></i></span></legend>
							<div class="accordion" id="accordion" role="tablist" aria-multiselectable="true">
								<?php if (!empty($evidoc_questions)) {
								    $counter = 1; ?>
									<?php foreach ($evidoc_questions as $section => $section_questions) {
									    $counter++; ?>
										<div class="panel">
								...
								<?php } else { ?>
									<div class="no-results">
										<div><?php echo $this->config->item('no_records'); ?></span> <span class="hide">&nbsp;Click the button to add a new Question <span><button title="Add new Question to this EviDoc type" style="width:8%" class="add-new-question btn btn-sm btn-success success-shadow"><i class="fas fa-plus-circle" style="font-size: 18px;"></i></button></span></span></div>							
									</div>
								<?php } ?>	
							</div>
						...
						<div class="modal-content">
							...
							<div class="modal-body">
								...
								<div class="input-group form-group">
									...
									<select id="evidoc_group" name="audit_group" class="form-control">
										...
										<?php if (!empty($evidoc_groups)) {
										    foreach ($evidoc_groups as $k => $group) { ?>
								...
								<div class="input-group form-group">
									...
									<select id="discipline_id" name="discipline_id" class="form-control required" data-label_text="Discipline" >
										...
										<?php if (!empty($disciplines)) {
										    foreach ($disciplines as $disp_key => $discipline) { ?>
									...
									<select id="asset_type_id" name="asset_type_id" class="form-control required" data-label_text="EviDocs Asset type" >
										<option value="" >Please Select a type</option>
											<?php if (!empty($asset_types)) {
											    foreach ($asset_types as $category => $asset_categories) { ?>
								...
								<div class="input-group form-group">
									...
									<select id="frequency_id" name="frequency_id" class="form-control required" data-label_text="Schedule Frequency" >
										...
										<?php if (!empty($schedule_frequencies)) {
										    foreach ($schedule_frequencies as $k => $shed_frequency) { ?>
                                ...
                                */ ?>
                                ...
                                <?php */ ?>
								<div class="form-group">
									<label class="input-group push-10-left">Evidoc Description</label>
