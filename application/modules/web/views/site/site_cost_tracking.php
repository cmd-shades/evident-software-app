<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel tile has-shadow">
			<legend>Building Cost Tracking <span class="pull-right"><span class="add-cost-item pointer" title="Click to add a new Cost item for this site"><i class="fas fa-plus-circle text-green"></i></span></legend>
...
<script>
	$(document).ready(function(){
		...
		$( '#add-cost-item-btn' ).click(function(){
			...
			$.ajax({
				url:"<?php echo base_url('webapp/site/add_cost_item/'); ?>",
				method:"POST",
				data:formData,
				dataType: 'json',
		...
		$( '.delete-cost-item' ).click(function(e){
			...
			swal({
				...
				if ( result.value ) {
					$.ajax({
						url:"<?php echo base_url('webapp/site/delete_cost_item/'.$site_details->site_id); ?>",
						method:"POST",
						data:{page:'details', site_id:siteID, cost_item_id:itemId},
						dataType: 'json',
						success:function( data ){
							if( data.status == 1 ){
								swal({
