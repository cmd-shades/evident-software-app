			...
			<?php if (!empty($site_panels)) { ?>
				<div class="row">
					<div class="accordion" id="panel-accordion" role="tablist" aria-multiselectable="true">
						<?php foreach ($site_panels as $k => $panel) {
						    $k++; ?>
							<div class="panel">
								<a style="color:<?php echo $panel->hex_color; ?>" class="panel-heading collapsed" role="tab" id="heading<?php echo number_to_words($k); ?><?php echo $k; ?>" data-toggle="collapse" data-parent="#panel-accordion" href="#collapse<?php echo number_to_words($k); ?><?php echo $k; ?>" aria-expanded="false" aria-controls="collapse<?php echo number_to_words($k); ?>">
									<h4 class="panel-title"><?php echo $panel->asset_name; ?> <?php echo (!empty($panel->asset_model)) ? ' - '.$panel->asset_model : ''; ?> <small><?php echo (!empty($panel->connected_assets)) ? '<em>('.count(object_to_array($panel->connected_assets)).' device'.((count(object_to_array($panel->connected_assets)) > 1) ? 's' : '').')</em>' : ''; ?></small><span class="pull-right" title="This Panel is <?php echo strtoupper($panel->event_tracking_status); ?>"><?php echo $panel->icon_class; ?></span></h4>
								</a>
													...
													<tbody>
														<?php foreach ($panel->connected_assets as $asset) { ?>
															<tr>
																...
																<td class="pull-right"><a href="<?php echo base_url('/webapp/asset/profile/'.$panel->asset_id); ?>"><span><i class="far fa-edit text-blue pointer"></i> &nbsp;view</span></a></td>
															</tr>
														<?php } ?>
													</tbody>
												</table>
