<?php if (!empty($category_sections)) {
    $counter = count($category_sections);
    foreach ($category_sections as $k => $category_section) {
        $k++; ?>
	...
	<div class="survey_creation_panel<?php echo $k; ?> col-md-4 col-md-offset-4 col-sm-12 col-xs-12" style="display:<?php echo ($k == 1) ? 'block' : 'none' ?>">
		<div class="x_panel tile has-shadow">
			...
			<?php if (!empty($category_questions->{$category_section})) {
			    foreach ($category_questions->{$category_section} as $key => $question) {
			        $key++; ?>
				...
				<div class="common-fields sectGrp<?php echo $sectGrp; ?>" style="display:block" >
					...
					<div class="form-group">
						...
						<?php } elseif (strtolower($question->response_type) == 'input' || strtolower($question->response_type) == 'datepicker') { ?>
						...
						<?php } elseif (strtolower($question->response_type) == 'text') { ?>
						...
						<?php } elseif (strtolower($question->response_type) == 'files') { ?>
						...
						<?php } elseif (strtolower($question->response_type) == 'select') { ?>
							...
							<select name="responses[questions][<?php echo $question->question_id; ?>][response]" class="form-control">
								...
								<?php if (!empty($question->response_options)) {
								    foreach ($question->response_options as $opt => $option) { ?>
