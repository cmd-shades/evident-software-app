<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel tile has-shadow">
			<legend>Vehicle Cost Tracking <span class="pull-right"><span class="add-cost-item pointer" title="Click to add a new Cost item for this site"><i class="fas fa-plus-circle text-green"></i></span></legend>
			<?php if (!empty($cost_tracking)) { ?>
				<table style="width:100%">
					...
					<?php if (!empty($cost_tracking)) {
					    foreach ($cost_tracking as $cost_item) { ?>
						<tr>
			...
			<div id="add-cost-item-modal" class="modal fade add-cost-item-modal" tabindex="-1" role="dialog" aria-hidden="true">
				<div class="modal-dialog modal-md">
					<form id="add-cost-item-form" >
						...
						<input type="hidden" name="vehicle_reg" value="<?php echo $vehicle_details->vehicle_reg; ?>" />
							...
							<div class="modal-body">
								...
								<div class="input-group form-group">
									...
									<select id="cost_item_type" name="cost_item_type" class="form-control required">
										...
										<?php if (!empty($cost_item_types)) {
										    foreach (array_to_object($cost_item_types) as $k => $item_type) { ?>
...
<script>
	$(document).ready(function(){
		...
		$( '#add-cost-item-btn' ).click(function(){
			...
			$.ajax({
				url:"<?php echo base_url('webapp/fleet/add_cost_item/'); ?>",
				method:"POST",
				data:formData,
				dataType: 'json',
		...
		// Cost item delete
		$( '.delete-cost-item' ).click(function(e){
			var vehicleReg = "<?php echo $vehicle_details->vehicle_reg; ?>";
			var itemId  = $( this ).data( 'cost_item_id' );
			swal({
				...
				if ( result.value ) {
					$.ajax({
						url:"<?php echo base_url('webapp/fleet/delete_cost_item/'.$vehicle_details->vehicle_reg); ?>",
						method:"POST",
						data:{page:'details', vehicle_reg:vehicleReg, cost_item_id:itemId},
						dataType: 'json',
						success:function( data ){
							if( data.status == 1 ){
								swal({
