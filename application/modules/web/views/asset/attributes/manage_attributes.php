					...
					<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
						<div class="row">
							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
								<a href="<?php echo base_url('/webapp/asset/new_attribute'); ?>" class="btn btn-block btn-success success-shadow" title="Click to add a new Asset Attribute"><i class="fas fa-plus-circle" title="" style="font-size: 18px;"></i></a><br/>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	...
	$( document ).ready(function(){
		...
		var search_str   	= null;
		var asset_groups_arr	= [];
		var start_index	 	= 0;
		var where 			= { 	
			'asset_group':asset_groups_arr
		};
		...
		function load_data( search_str, where, start_index ){
			$.ajax({
				url:"<?php echo base_url('webapp/asset/attributes_lookup'); ?>",
				method:"POST",
		...
		function get_statuses( identifier ){
			...
			if( identifier == '.asset-groups' ){
				...
				$( identifier ).each(function(){
					...
					if( $(this).is(':checked') ){
						totalChekd++;
						asset_groups_arr.push( $(this).val() );
					}else{
						unChekd++;
					}
				});
