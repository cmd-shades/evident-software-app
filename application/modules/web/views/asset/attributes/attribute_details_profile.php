				...
				<div class="row alert alert-ssid bg-blue" role="alert">
					<div class="profile-overview">
						<div class="col-md-12 col-sm-12 col-xs-12">
							<legend>Attribute Profile <span class="pull-right"><span class="edit-attribute pointer hide" title="Click to edit thie Asset Type profile"><i class="fas fa-pencil-alt"></i></span> &nbsp; <span class="delete-attribute-btn pointer" title="Click to delete this Asset Type profile" ><i class="far fa-trash-alt"></i></span></span></legend>
							<div class="row">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<table style="width:100%;">
										<tr>
											<td width="15%"><label><strong>No. Associated Asset Type</strong></label></td>
											<td width="85%"><?php echo !empty($associated_asset_types) ? count($associated_asset_types, 1) : 0; ?></td>
										</tr>
										<tr>
					...
					<div class="row">
						<div class="col-md-6 col-sm-6 col-xs-12">
							<div class="x_panel tile has-shadow">
								<form id="update-asset-form" class="form-horizontal">
									<input type="hidden" name="page" value="details" />
									<input type="hidden" name="attribute_id" value="<?php echo $attribute_details->attribute_id; ?>" />
									<input type="hidden" name="asset_type_id" value="<?php echo !empty($attribute_details->asset_type_id) ? $attribute_details->asset_type_id : null; ?>" />
	...
	$( document ).ready( function(){
		...
		$( '.update-attribute-btn' ).click( function( event ){
			...
			swal({
				...
				if ( result.value ) {
					$.ajax({
						url:"<?php echo base_url('webapp/asset/update_asset_type_attribute/'.$attribute_details->attribute_id); ?>",
						method:"POST",
						data:formData,
						dataType: 'json',
		...
		$( '.delete-attribute-btn' ).click( function( event ){
			...
			swal({
				...
				html: 'This is an irreversible action and will affect Asset Types that are linked to it!',
				showCancelButton: true,
				confirmButtonColor: '#5CB85C',
				cancelButtonColor: '#9D1919',
				confirmButtonText: 'Yes'
				...
				if ( result.value ) {
					$.ajax({
						url:"<?php echo base_url('webapp/asset/delete_asset_type_attribute/'.$attribute_details->attribute_id); ?>",
						method:"POST",
						data:formData,
						dataType: 'json',
						success:function(data){
							if( data.status == 1 ){
								...
								window.setTimeout(function(){
									window.location.href = "<?php echo base_url('webapp/asset/attributes'); ?>";
								} ,3000);
							}else{
								swal({
									type: 'error',
