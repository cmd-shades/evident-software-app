...
<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel tile has-shadow">
			<legend>Review Pending Jobs Upload</legend>
			<?php if (!empty($pending_jobs)) { ?>
				<div class="row">
					<div class="col-md-12">
						<div style="overflow-y: hidden;" >
							<?php foreach ($pending_jobs as $group => $records) { ?>
								<form id="frm-<?php echo $group; ?>" method="POST" >
									<input type="hidden" name="account_id" value="<?php echo $this->user->account_id; ?>" />
									<input type="hidden" name="page" value="details"/>
									<table width="100%" >								
										<tr>
											<td width="80%" style="color:<?php echo ($group == 'new-records') ? 'green' : 'red'; ?>" title="These records <?php echo ($group == 'new-records') ? 'are ready for processing into Job records' : ((($group == 'existing-records')) ? 'already exist on the system' : 'are missing the Emails'); ?>" ><strong><span class="pointer grp" data-grp_id="<?php echo $group;?>"><?php echo strtoupper(str_replace('-', ' ', $group)); ?> ( <?php echo (!empty($records)) ? count($records) : '0'; ?> )</span></strong></td>
											<td width="20%" >
												<span class="pull-right">
													<button style="display:none" class="submit-records btn btn-sm btn-block btn-success btn-<?php echo ($group == 'new-records') ? 'success' : 'danger'; ?> grp_<?php echo $group;?>" data-action_type="<?php echo ($group == 'new-records') ? 'add' : 'remove'; ?>" data-form_id="<?php echo $group; ?>" >
											...
											<td colspan="2" width="100%" >
												<table class="table table-responsive" style="width:100%" >
													<tr>
														...
														<td class="text-bold" width="30%">Address</td>
														<td class="text-bold" width="30%">Job Type</td>
													...
													<tbody>
														<?php foreach ($records as $key => $record) { ?>
															<tr data-temp_job_id="<?php echo $record['temp_job_id']; ?>" >
																...
																<input type="hidden" name="jobs_data[<?php echo $record['temp_job_id']; ?>][contract_name]" value="<?php echo $record['contract_name'];?>" />
																...
																<td width="20%" style="border-bottom: 1px solid #fff;">
																	<select class="form-control changeable-field" name="jobs_data[<?php echo $record['temp_job_id']; ?>][address_id]" >
																...
																<td width="25%" style="border-bottom: 1px solid #fff;">
																	<select class="form-control changeable-field" name="jobs_data[<?php echo $record['temp_job_id']; ?>][job_type_id]" >
																		<option value="" >Select Job Type</option>
																		<?php if (!empty($record['job_types'])) {
																		    foreach (json_decode($record['job_types']) as $jt => $job_type) { ?>
			...
			<?php } else { ?>
				<div class="row">
					...
					<div class="col-md-3">
						<a href="<?php echo base_url('webapp/job/upload_jobs'); ?>" class="btn btn-sm btn-block btn-info" type="submit" >Start Upload Job</a>					
					</div>
				</div>
			<?php } ?>
		</div>
