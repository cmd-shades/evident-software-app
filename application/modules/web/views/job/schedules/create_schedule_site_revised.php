		...
		<div class="alert bg-blue no-border" role="alert">
			<table class="table-responsive" style="width:100%" >
				<tr>
					...
					<th width="25%" ><legend>ESTATE NAME</legend></th>
					<th width="30%" ><legend>BUILDING ADDRESS</legend></th>
					<th width="20%" ><legend>COMPLIANCE STATUS</legend></th>
				</tr>
				<tr>
	...
	<div class="x_panel tile has-shadow">
		<div class="schedule_creation_panel1 col-md-12 col-sm-12 col-xs-12">
			...
			<div class="form-group" >
				<select id="frequency_id" name="frequency_id" class="form-control required" data-label_text="Schedule Frequency" >
					...
					<?php if (!empty($schedule_frequencies)) {
					    foreach ($schedule_frequencies as $k => $frequency) { ?>
		...
		<div class="schedule_creation_panel2 col-md-12 col-sm-12 col-xs-12" style="display:none" >
			...
			<div class="row">
				...
				<div>
					<div class="row">
						<?php if (!empty($building_categories)) {
						    foreach ($building_categories as $category => $category_data) { ?>
						...
						<?php }
							...
							<div class="categories-container">
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="x_panel" >
										<div>There's currently no Categories available for this Site.</div>
										<small>Please create Assets for this Building before creating the Schedule for them.</small>
									</div>
								</div>
							</div>
						<?php } ?>
		...
		<form id="schedule-creation-form" >
			...
			<input type="hidden" name="address_id" value="<?php echo (!empty($site_details->site_address_id)) ? $site_details->site_address_id : false; ?>" />
...
<script>
	...
	$( document ).ready( function() {
		...
		//Restrict Selection of Category to a single item
		$( '.categories-container' ).on( 'click', 'input[type="checkbox"]', function() {      
			$('.categories-container input[type="checkbox"]').not(this).prop('checked', false);      
		});		
		
		//Restrict Selection of Asset Types to a single item
		$( '#assets_by_asset_type_category' ).on( 'click', 'input[type="checkbox"]', function() {      
			$('#assets_by_asset_type_category input[type="checkbox"]').not(this).prop('checked', false);      
		});	
		...
		function submitScheduleForm( liMit = "<?php echo SCHEDULE_CLONE_DEFAULT_LIMIT; ?>", offSet = 0 ){
			...
			var selectedEvidocType 	= $( '#evidoc_type_id .evidoc-type-select option:selected' ).val();
			var selectedJobType 	= $( '#job_type_id .job-type-options option:selected' ).val();
			var selectedCategories	= '';
			
			$( "[name='category_id[]']" ).each( function(){
				if( $( this ).is( ':checked' ) ){
					selectedCategories = $( this ).val(); //Rewrite this line to allow multiples. This restricts to one everytime
				}
			});
			...
			$( '#schedule-creation-form' ).prepend( '<input type="hidden" name="category_id" value="'+selectedCategories+'" /><input type="hidden" name="total_activities_due" value="'+totalactivitiesDue+'" />' );
			$( '#schedule-creation-form' ).prepend( '<input type="hidden" name="job_type_id" value="'+selectedJobType+'" /><input type="hidden" name="total_activities_due" value="'+totalactivitiesDue+'" />' );
			$( '#schedule-creation-form' ).prepend( '<input type="hidden" name="evidoc_type_id" value="'+selectedEvidocType+'" /><input type="hidden" name="total_activities_due" value="'+totalactivitiesDue+'" />' );
			...
			swal({
				...
				if ( result.value ) {
					$.ajax({
						url:"<?php echo base_url('webapp/job/create_schedules_revised/'); ?>",
						method:'POST',
						data:formData,
						dataType: 'json',
						beforeSend: function(){
						...
						success:function( data ){
							...
							if( data.status == 1 && ( data.schedules.schedule_id !== '' ) ){
								...
								var newScheduleId 		= data.schedules.schedule_id,
									scheduleName 		= data.schedules.schedule_name,
									totalExpectedAssets = data.schedules.processed_assets.length,
									totalExpectedJobs 	= data.schedules.processed_activities.length;

								if( totalExpectedAssets !== 0 ){
									
									buildingName = "<?php echo (!empty($site_details->site_name)) ? $site_details->site_name : ''; ?>";
									...
									swal({
										...
										title: 'Schedule Processing started!',
										html:
											'<p>Please check and confirm that the details below are correct, then click proceed to complete the processing.</p>' +
											'<table class="table table-responsive pull-left">' +
												'<tr>' +
													'<th>Building Name:</th><td>' + buildingName+ '</td>' +
												'</tr>' +
												'<tr>' +
													'<th>Schedule Name:</th><td>' + scheduleName+ '</td>' +
												'</tr>' +
												'<tr>' +
													'<th>Total Assets:</th><td>' + totalExpectedAssets+ '</td>' +
												'</tr>' +
												'<tr>' +
													'<th>Total Jobs <small>(expected)</small>:</th><td>' + totalExpectedJobs + '</td>' +
												'</tr>' +
											'</table>'
									}).then( function (result) {
										if ( result.value ) {
											//Do something here
											completeScheduleProcessingRevised({
												page: 'details',
		...
		$( '.prepare-schedule-placeholders' ).click( function(){
			...
			$.ajax({
				url:"<?php echo base_url('webapp/job/prepare_job_placeholders/'); ?>",
				method:"POST",
		...
		**/
		...
		/*
		* Compelete Schedule Processing 
		**/
		function completeScheduleProcessingRevised( formData ){
		
			siteId		 = "<?php echo (!empty($site_details->site_id)) ? $site_details->site_id : '0'; ?>";
			var formData = formData;

			if( ( formData.schedule_id.length > 0 ) && ( siteId !== '0' ) ){
				$.ajax({
					url:"<?php echo base_url('webapp/job/complete_schedule_processing_revised'); ?>",
					method:'POST',
					data:formData,
					dataType: 'json',
					beforeSend: function(){
