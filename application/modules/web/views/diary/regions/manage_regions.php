					...
					<table id="datatable" class="table table-responsive" style="margin-bottom:0px;width:100%" >
						<thead>
							<tr>
								...
								<th width="25%">Region Description</th>
								<th width="45%">Region Postcodes</th>
...
<script type="text/javascript">
	...
	$( document ).ready(function(){
		...
		var search_str   	= null;
		var region_groups_arr	= [];
		var start_index	 	= 0;
		var where 			= { 	
			'region_group':region_groups_arr
		};
		...
		function load_data( search_str, where, start_index ){
			$.ajax({
				url:"<?php echo base_url('webapp/diary/regions_list'); ?>",
				method:"POST",
		...
		function get_statuses( identifier ){
			...
			if( identifier == '.region-groups' ){
				...
				$( identifier ).each(function(){
					...
					if( $(this).is(':checked') ){
						totalChekd++;
						region_groups_arr.push( $(this).val() );
					}else{
						unChekd++;
					}
				});
