...
class AuditModel extends Model
{
	 ...
	 */
	private DocumentHandlerModel $document_service;
	/**
	 * @var mixed $audit_group
	 */
	private $audit_group;
            ...
            if ($query->num_rows() > 0) {
                if ($un_grouped) {
                    ...
                    foreach ($query->result() as $k => $row) {
                        ...
                        } elseif (!empty($group_by_category)) {
                            $data[$row->category_name][] 	= $row;
                        } elseif (!empty($group_by_frequesncy)) {
            ...
            switch(strtolower($stat_type)) {
                case 'completion':
                    ...
                    $query = $this->db->select(
                        'SUM( CASE WHEN audit_status = "In Progress"  THEN 1 ELSE 0 END ) AS `In Progress`,
    ...
    public function reorder_evidoc_questions($account_id = false, $evidoc_type_id = false, $postdata = false)
    {
        ...
        if (!empty($account_id) && !empty($evidoc_type_id)) {
            ...
            if (!empty($questions_data)) {
                ...
                } elseif ($result && ($total_questions != $updated_questions)) {
