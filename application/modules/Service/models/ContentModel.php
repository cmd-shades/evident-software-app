...
{
	/**
	 * @var \Application\Modules\Service\Models\ProviderModel $provider_service
	 */
	private $provider_service;
	/**
	 * @var \Application\Modules\Service\Models\EaselApiModel $easel_service
	 */
    ...
    public function get_clearance($account_id = false, $clearance_id = false, $where = false, $limit = DEFAULT_LIMIT, $offset = DEFAULT_OFFSET)
    {
        ...
        if (!empty($account_id)) {
            ...
            if (!empty($where)) {
                ...
                } elseif (!empty($where['territory_id'])) {
            ...
            if (!empty($batch)) {
                foreach ($batch as $key => $row) {
                    if ($row['checked'] == 1) {
                        ...
                        } elseif (!empty($row['clearance_start_date'])) {
                ...
                foreach ($query->result() as $k => $row) {
                    if (!empty($row->license_start_date)) {
                        ...
                        if (($row->content_in_use == 1)) {
                            ...
                            } elseif (($row->content_in_use != 1) && ($elaspsed_months > $aging_period)) {
                                $result->content_group          = 'Library';
