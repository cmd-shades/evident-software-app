<?php if ( !defined('BASEPATH') ) exit('No direct script access allowed');
	...
	class SIDD_PDF extends TCPDF {
		...
		public function renderImages( $doc_type = false, $images = false ){
			...
			if( !empty( $doc_type ) && !empty( $images ) ){
				switch( $doc_type ){
					case 'audit':
						$html_str = '<div style="color:#696969"><div><h3 style="font-size:90%" >Audit Photos</h3><hr></div>';
						$html_str .= '<table text-align="center" style="text-align:center; font-family: Arial, Helvetica, sans-serif;font-size:12px; width:100%; padding:15px;" >';
							...
							foreach( $images  as $segment => $segment_files ){
								foreach( $segment_files  as $q => $file ){
									$html_str .= '<tr>';
										$html_str .= '<td width="25%" >&nbsp;</td>';
										$html_str .= '<td width="50%" style="text-align:center;" ><img src="'.$file->document_link.'" /><br><span>'.$segment.' - '.$file->document_name.'</span></td>';
										$html_str .= '<td width="25%" >&nbsp;</td>';
									$html_str .= '</tr>';
									// $html_str .= '<tr>';
										// $html_str .= '<td width="15%" >&nbsp;</td>';
										// $html_str .= '<td width="70%" ><strong>Document name:</strong> '.$segment.' - '.$file->document_name.'</td>';
										// $html_str .= '<td width="15%" >&nbsp;</td>';
									// $html_str .= '</tr>';
								}
							}
		...
		public function Header() {
			// Logo
			$img_file= $this->appDir.'/assets/images/logos/main-logo-small.png';
			$this->Image( $img_file, 94, 10, 15, '', 'PNG', '', 'C', false, 300, '', false, false, 0, false, false, false);
	...
	if( !empty( $html_content ) ){
		...
		//$pdf->Output( $document_setup['generic_details']['document_name'].'.pdf', 'D'); //Download
		$pdf->Output( $document_setup['generic_details']['document_name'].'.pdf', 'I'); //Preview
	}
